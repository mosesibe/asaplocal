...
export async function POST(req: NextRequest) {
  ...
  const { name, isRegulatedTrade, suggestedQualifications } = await req.json();
  ...
  const category = await prisma.category.create({
    data: {
      name,
      slug,
      isRegulatedTrade: !!isRegulatedTrade,
      suggestedQualifications: Array.isArray(suggestedQualifications) ? suggestedQualifications : [],
    },
  });
