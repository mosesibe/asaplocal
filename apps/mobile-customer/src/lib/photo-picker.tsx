 ...
 * spot in the app (avatar, dispute photos, studio inspiration, message
 * attachments) — previously each of those only offered the photo library,
 * with no way to snap a new photo on the spot. Camera capture is inherently
 * one-shot, so `allowsMultipleSelection`/`selectionLimit` in `options` only
 * apply to the library branch; the camera always returns a single asset.
 *
